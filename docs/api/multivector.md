...
`MultiVector` 是几何代数中的核心数据类型，表示多重向量（标量、向量、二重向量等的线性组合）。
...
**参数**:
- `config` - 代数配置
- `coefficients` - 所有基的系数数组

**返回**: MultiVector

**示例**:
```python
# 3D 中系数顺序: [标量, e1, e2, e3, e12, e13, e23, e123]
...
**参数**: `r` - 阶次 (0=标量, 1=向量, 2=二重向量, ...)

**返回**: 指定阶次的 MultiVector
...
对 r 阶次元素乘以 (-1)^(r(r-1)/2)。

**示例**:
```python
v = alg.vector([1, 2, 3])
rev = v.reversion()  # v (向量不变)

B = e1 ^ e2
B_rev = B.reversion()  # -B (二重向量取反)
```
...
# 验证
product = v * v_inv
print(product.scalar_part())  # 接近 1
```
...
reflected = v.reflect_in(e1)  # 在 yz 平面反射
# x 分量取反
```
